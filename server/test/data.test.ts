...
// Every route under test sits behind requireAuth and scopes its queries to the session
// user, so each test gets a fresh account and drives the API as that account.
let user: TestUser;
...
describe('Notebooks CRUD', () => {
  ...
  it('patches a notebook and 404s for an unknown id', async () => {
    ...
    const res = await api.patch(`/api/notebooks/${nb.id}`).send({ name: 'Renamed', archived: true });
    expect(res.status).toBe(200);
    expect(res.body.notebook.name).toBe('Renamed');
    expect(res.body.notebook.archived).toBe(true);

    const missing = await api.patch('/api/notebooks/nope').send({ name: 'x' });
  ...
  it('deletes a notebook and cascades its notes', async () => {
    ...
    const res = await api.delete(`/api/notebooks/${nb.id}`);
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ ok: true });

    const noteRes = await api.get(`/api/notes/${note.id}`);
...
describe('Notes CRUD + tags', () => {
  it('creates a note with tags and returns NoteLite-shaped fields', async () => {
    ...
    const res = await api.post('/api/notes').send({ notebookId: nb.id, title: 'My Note', tags: ['week1', 'lecture'] });
  ...
  it('400s creating a note with an unknown notebookId', async () => {
    const res = await api.post('/api/notes').send({ notebookId: 'does-not-exist' });
    expect(res.status).toBe(400);
  });
  ...
  it('lists notes filtered by notebookId and tag, respecting the archived flag', async () => {
    ...
    await api.patch(`/api/notes/${a.id}`).send({ archived: true });

    const activeRes = await api.get(`/api/notes?notebookId=${nb.id}`);
    expect(activeRes.body.total).toBe(1);
    expect(activeRes.body.notes[0].title).toBe('B');

    const archivedRes = await api.get(`/api/notes?notebookId=${nb.id}&archived=1`);
    expect(archivedRes.body.total).toBe(1);
    expect(archivedRes.body.notes[0].title).toBe('A');

    const tagRes = await api.get('/api/notes?tag=week2');
  ...
  it('updates a note (title/pinned) via PATCH', async () => {
    const nb = await createNotebook();
    const note = await createNote(nb.id);
    const res = await api.patch(`/api/notes/${note.id}`).send({ title: 'Updated title', pinned: true });
    expect(res.status).toBe(200);
  ...
  it('404s GET/PATCH/DELETE for an unknown note', async () => {
    expect((await api.get('/api/notes/nope')).status).toBe(404);
    expect((await api.patch('/api/notes/nope').send({ title: 'x' })).status).toBe(404);
  ...
  it('deletes a note', async () => {
    ...
    expect(res.status).toBe(200);
    expect((await api.get(`/api/notes/${note.id}`)).status).toBe(404);
  });
  ...
  it('recent notes excludes archived and orders by updatedAt desc', async () => {
    ...
    await db.prepare('UPDATE notes SET updated_at = ? WHERE id = ?').run(new Date(Date.now() - 3000).toISOString(), a.id);
    await db.prepare('UPDATE notes SET updated_at = ? WHERE id = ?').run(new Date(Date.now() - 2000).toISOString(), b.id);
    await db.prepare('UPDATE notes SET updated_at = ? WHERE id = ?').run(new Date(Date.now() - 1000).toISOString(), c.id);
    await api.patch(`/api/notes/${a.id}`).send({ archived: true });

    const res = await api.get('/api/notes/recent');
...
describe('Version snapshot policy', () => {
  it('creates an autosave version (capturing pre-edit state) on the first content-changing PATCH', async () => {
    ...
    await api.patch(`/api/notes/${note.id}`).send({ title: 'V2' });

    const versions = await api.get(`/api/notes/${note.id}/versions`);
  ...
  it('does not snapshot again within the 10-minute autosave window', async () => {
    ...
    const note = await createNote(nb.id, { title: 'V1' });
    await api.patch(`/api/notes/${note.id}`).send({ title: 'V2' });
    await api.patch(`/api/notes/${note.id}`).send({ title: 'V3' });
  ...
  it('snapshots again once the latest version is older than 10 minutes', async () => {
    ...
    await db.prepare('UPDATE note_versions SET created_at = ? WHERE note_id = ?').run(
  ...
  it('POST /versions always creates a manual snapshot, and restore snapshots current state first', async () => {
    ...
    const snap = await api.post(`/api/notes/${note.id}/versions`).send({ label: 'Checkpoint' });
    expect(snap.status).toBe(201);
    expect(snap.body.version.cause).toBe('manual');
    expect(snap.body.version.label).toBe('Checkpoint');

    await api.patch(`/api/notes/${note.id}`).send({ title: 'Changed' });

    const restore = await api.post(`/api/notes/${note.id}/restore/${snap.body.version.id}`);
  ...
  it('404s restoring an unknown version', async () => {
    const nb = await createNotebook();
    const note = await createNote(nb.id);
    const res = await api.post(`/api/notes/${note.id}/restore/999999`);
...
describe('Wikilinks and backlinks', () => {
  it('extracts [[wikilinks]] into links, resolving titles case-insensitively', async () => {
    ...
    const detail = await api.get(`/api/notes/${source.id}`);
  ...
  it('replaces links on re-save rather than accumulating them', async () => {
    ...
    await api.patch(`/api/notes/${source.id}`).send({ contentText: 'Now links to [[Note B]] instead.' });
  ...
  it('unlinked-mentions finds a plain-text title mention that is not yet linked', async () => {
    ...
    const res = await api.get(`/api/notes/${target.id}/unlinked-mentions`);
    expect(res.status).toBe(200);
...
describe('Search', () => {
  it('returns a bm25-ranked, highlighted snippet for a matching query', async () => {
    ...
    const res = await api.get('/api/search?q=complexity');
    expect(res.status).toBe(200);
  ...
  it('search/titles does contains matching for the quick switcher', async () => {
    ...
    const res = await api.get('/api/search/titles?q=Operating');
    expect(res.status).toBe(200);
...
describe('Dashboard', () => {
  it('returns the full aggregated dashboard shape', async () => {
    ...
    await api.patch(`/api/notes/${note.id}`).send({ pinned: true });
...
describe('Export', () => {
  it('exports a note as Markdown containing its heading and body text', async () => {
    ...
    const res = await api.get(`/api/notes/${note.id}/export?format=markdown`);
    expect(res.status).toBe(200);
  ...
  it('400s for an unsupported export format', async () => {
    const nb = await createNotebook();
    const note = await createNote(nb.id);
    const res = await api.get(`/api/notes/${note.id}/export?format=pdf`);
    expect(res.status).toBe(400);
  });
});
