# Folio — Product Specification (SPEC.md)
...
- **Empty dashboard (brand-new install)**: onboarding card — "Add your first notebook" with a one-field emoji+name form, seeded suggestion chips (e.g. common CS module names) to remove blank-canvas hesitation.
- **Empty Ask-my-notes corpus**: "You haven't written any notes yet — Folio answers from what you've written, so there's nothing to draw on. Take a few notes first." (never a raw error).
...
11. **The single-user/no-auth decision is documented, not silent.** The README/settings page states plainly that Folio is intended for trusted local-network use only and is not hardened against untrusted network access.
