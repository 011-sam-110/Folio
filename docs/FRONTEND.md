...
**Editor page `/note/:noteId`** (web-editor) is the crown jewel. Breadcrumb (notebook › note), autosave state ("Saving…" → "Saved · 2m ago" with cloud check), actions bar: Pin, AI ▾ (Improve writing, Summarize, Generate flashcards, Suggest title), Import into this note ▾, History, Export Markdown, Info (word count, backlinks count).
