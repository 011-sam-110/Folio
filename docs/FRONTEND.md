...
**Editor page `/note/:noteId`** (web-editor) is the crown jewel. Breadcrumb (notebook › note), then the action bar (`NoteActionBar.tsx`) in three zones, every control text-labelled: **Write** — `＋ Insert ▾` (the one filled/primary button) and `✦ AI ▾` (Improve writing, Clean up formatting, Find missing content from uploads *[disabled - no endpoint yet]*, Generate flashcards → 5/8/12); **Panels** — one segmented control of toggles, each carrying `aria-pressed`: Outline, Comments (+ unresolved-count badge), Ink, Find, Assistant; **Note actions** — Share, then `⋯ More ▾` grouped by hairline into *This note* (History, Export .md, Pin/Unpin, Note info → word/char count, created, updated, backlinks), *AI* (Summarise, Suggest a title) and *Import into note* (Photo of notes, Slides, Transcript), then the autosave chip ("Saving…" → "Saved · 2m ago", "Save failed" + Retry). The whole AI menu and the *AI* group in More are gated on live gateway availability (`useAiAvailable`).
