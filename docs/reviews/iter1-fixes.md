...
**#10 CORS wide open.** The origin callback now allows only localhost + private-LAN
...
**#18 Mobile drawer.** The closed state is `inert` + `visibility: hidden` (transition-delayed so
...
**#25 Dashboard mobile.** At <640px: `.note-grid`/`.notebook-col-row` collapse to one column,
...
ellipsis need block boxes. `display: block` added, and full-text results now elide cleanly.
